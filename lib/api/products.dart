...
import 'package:myproject/model/detailsproduct/respone_product_details.dart';
...
class ApiServiceProducts {
 ...
 Future<ProductDataModel> getDetailProduct(String productId) async {
  ...
  Future<ProductDataModels> getDetailsProduct(String productId) async {
    // await CheckToken.checkExpireToken();
    ...
    if (response.statusCode == 201) {
      var responseData = json.decode(response.body);
      if (responseData['success'] == true) {
      var response = ReponseProductDetails.fromJson(responseData);
      return response.data!;
      } else {
        throw Exception(responseData['message']);
      }
